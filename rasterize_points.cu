...
//   torch::Tensor out_alpha = torch::full({1, H, W}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		inside.contiguous().data<bool>(),
		debug);
  }
//   return std::make_tuple(rendered, out_color, out_depth, out_depth_var, out_median_depth, out_normal, out_objects, out_alpha, radii, geomBuffer, binningBuffer, imgBuffer);
//   return std::make_tuple(rendered, out_color, out_depth, out_normal, out_objects, out_alpha, radii, geomBuffer, binningBuffer, imgBuffer);
	...
	const bool f_count)
{
  ...
  if(P != 0)
  {
	...
	// overloaded forward, added gaussians_count, important_score
	  rendered = CudaRasterizer::Rasterizer::forwardVisi(
	    geomFunc,
		binningFunc,
		imgFunc,
	    P, degree, M,
	...
	const bool f_count)
{
  ...
  if(P != 0)
  {
	...
	// overloaded forward, added gaussians_count
	  rendered = CudaRasterizer::Rasterizer::forwardVisiAcc(
	    geomFunc,
		binningFunc,
		imgFunc,
	    P, degree, M,
  ...
  return std::make_tuple(gaussians_count, rendered, radii, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dxyz_view = torch::zeros({P, 3}, means3D.options());
  ...
  {  
	...
	//   dL_dout_alpha.contiguous().data<float>(),
	  ...
	  dL_dxyz_view.contiguous().data<float>(),
  ...
  return std::make_tuple(dL_dmeans2D, dL_dmeans2D_densify, dL_dcolors, dL_dnormals, dL_dobjects, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations);
}
