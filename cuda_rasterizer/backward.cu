	...
	glm::vec4* dL_drot)
{
	...
	if (dirs == nullptr) {
		...
		dL_dmean2.x = view[0] * dL_dxyz_view[idx].x + view[1] * dL_dxyz_view[idx].y + view[2] * dL_dxyz_view[idx].z;
		dL_dmean2.y = view[4] * dL_dxyz_view[idx].x + view[5] * dL_dxyz_view[idx].y + view[6] * dL_dxyz_view[idx].z;
		dL_dmean2.z = view[8] * dL_dxyz_view[idx].x + view[9] * dL_dxyz_view[idx].y + view[10] * dL_dxyz_view[idx].z;
	}
...
renderCUDA(
	...
	float3* __restrict__ dL_dxyz_view,
...
)
{
	...
	float T = T_final;
	const float weight_final = inside ? alpha[pix_id] : 1; // 1 ??
	const float depth_final = inside ? depth[pix_id] : 1;


	const float final_D = 0;
	const float final_D2 = 0;
	const float final_A = 0;
	const float dL_dreg = 0;
	float last_dL_dT = 0;
	if (NUM_DIST) {
	...
	float accum_depth_rec2 = 0;
	float dL_dpixel_depth;
	float accum_depth1_rec = 0;
	float accum_depth2_rec = 0;
	float dL_dpixel_depth1;
	float dL_dpixel_depth2;
	float accum_normal_rec[3] = { 0 };
	float accum_normal_rec2[3] = { 0 };
	...
	if (inside) {
		...
		dL_dpixel_depth = dL_dpixels[DEPTH_OFFSET * H * W + pix_id];
		if (NUM_DEPTH2) {
			dL_dpixel_depth1 = dL_dpixels[DEPTH2_OFFSET * H * W + pix_id];
			dL_dpixel_depth2 = dL_dpixels[(DEPTH2_OFFSET + 1) * H * W + pix_id];
		}
		for (int i = 0; i < 3; i++)
			dL_dpixel_normal[i] = dL_dpixels[(NORMAL_OFFSET + i) * H * W + pix_id];
		if (objects != nullptr) {
			for (int i = 0; i < O; i++)
				dL_dpixel_obj[i] = dL_dpixels[(OBJECT_OFFSET + i) * H * W + pix_id];
		}
		dL_dalpha = dL_dpixels[ALPHA_OFFSET * H * W + pix_id];
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const float3 normal = {collected_normals[j], collected_normals[BLOCK_SIZE + j], collected_normals[2 * BLOCK_SIZE + j]};
			float c_d;
			if (dirs != nullptr) {
				c_d = intersect(xyz, normal, dir_pix);
			}
			else {
				c_d = collected_depths[j];
			}
			if (c_d < NEAR_PLANE) 
				continue;
			...
			if (inside_g) {
				...
				// float dL_dd_j = dchannel_ddepth * dL_dpixel_depth;
				// mean depth: D the rendered depth; B final_weight
				dL_dopa += (((c_d - accum_depth_rec) - depth_final * (1 - accum_depth_rec2)) / weight_final) * dL_dpixel_depth;
				float dL_dd_j = (dchannel_ddepth / weight_final) * dL_dpixel_depth;


				if (NUM_DEPTH2) {
					const float depth_ndc = (FAR_PLANE * c_d - FAR_PLANE * NEAR_PLANE) / ((FAR_PLANE - NEAR_PLANE) * c_d);
					float dmax_t_dd = (FAR_PLANE * NEAR_PLANE) / ((FAR_PLANE - NEAR_PLANE) * c_d * c_d);

					// ndc depth D1
					dL_dd_j += dchannel_ddepth * dmax_t_dd * dL_dpixel_depth1;

					// ndc depth ** 2 D2
					dL_dd_j += 2.0f * dchannel_ddepth * depth_ndc * dmax_t_dd * dL_dpixel_depth2;
						...
						// it will bia toward creating extragated Gaussians near front
						// detach weight
						accum_depth1_rec = last_alpha * depth_ndc + (1.f - last_alpha) * accum_depth1_rec;
						dL_dopa += (c_d - accum_depth1_rec) * dL_dpixel_depth1;
						
						accum_depth2_rec = last_alpha * depth_ndc * depth_ndc + (1.f - last_alpha) * accum_depth2_rec;
						dL_dopa += (c_d - accum_depth2_rec) * dL_dpixel_depth2;
					#endif
				}
				...
				if (NUM_DIST) {
					...
					dL_dd_j += 2.0f * (T * alpha) * (depth_ndc * final_A - final_D) * dL_dreg * dmax_t_dd;
					...
					float dL_dweight = 0.0f;
						...
						dL_dweight += (final_D2 + depth_ndc * depth_ndc * final_A - 2 * depth_ndc * final_D) * dL_dreg; // * one_div_square_one_minus_T;
				...
				float pro_dot;
				float3 dL_dnormal_depth;
				float3 dL_dxyz_view_single;
				if (dirs != nullptr) {
					pro_dot = dot(normal, dir_pix);
					dL_dnormal_depth = dL_dd_j * dir_pix.z * (pro_dot * xyz - dir_pix * (dot(normal, xyz))) / (pro_dot * pro_dot);
					dL_dxyz_view_single = dL_dd_j * dir_pix.z * normal / pro_dot;
					atomicAdd(&(dL_dxyz_view[global_id].x), dL_dxyz_view_single.x);
					atomicAdd(&(dL_dxyz_view[global_id].y), dL_dxyz_view_single.y);
					atomicAdd(&(dL_dxyz_view[global_id].z), dL_dxyz_view_single.z);
				} else {
					atomicAdd(&(dL_ddepths[global_id]), dL_dd_j);
				}

				// Propagate gradients from pixel normal to opacity
				for (int ch = 0; ch < 3; ch++)
				{
					const float n = collected_normals[ch * BLOCK_SIZE + j];
					// const float n = index(normal, ch);
					// Update last normal (to be used in the next iteration)
					accum_normal_rec[ch] = last_alpha * last_normal[ch] + (1.f - last_alpha) * accum_normal_rec[ch];
					accum_normal_rec2[ch] = last_alpha + (1.f - last_alpha) * accum_normal_rec2[ch];
					last_normal[ch] = n;

					const float dL_dchannel = dL_dpixel_normal[ch];
					// dL_dopa += (n - accum_normal_rec[ch]) * dL_dchannel;
					dL_dopa += (((n - accum_normal_rec[ch]) - last_normal[ch] * (1 - accum_normal_rec2[ch])) / weight_final) * dL_dchannel;
					// Update the gradients w.r.t. normal of the Gaussian. 
					// Atomic, since this pixel is just one of potentially
					// many that were affected by this Gaussian.
					// atomicAdd(&(dL_dnormals[global_id * 3 + ch]), dchannel_dnormal * dL_dchannel);
					atomicAdd(&(dL_dnormals[global_id * 3 + ch]), (dchannel_dnormal / weight_final) * dL_dchannel);
					if (dirs != nullptr)
					{
						atomicAdd(&(dL_dnormals[global_id * 3 + ch]), index(dL_dnormal_depth, ch));
					}
				}
			}
			...
			// Update gradients for densification: have problem
