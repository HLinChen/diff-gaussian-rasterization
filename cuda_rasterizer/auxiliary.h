...
#define OBJECT_OFFSET 8
#define DISTORTION_OFFSET 10
#define DEPTH2_OFFSET 10
#define OUTPUT_CHANNELS 3 + 1 + 3 + 1 + NUM_OBJECTS + NUM_DIST + NUM_DEPTH2 // RGB + depth + normal + alpha+ object ids + distortion 
...
// ------------------------ FLOAT3/4 UTILS ------------------------
__forceinline__ __device__ float index(const float3 &v, int idx) {
    // 通过switch语句来根据索引访问对应的成员
    switch(idx) {
        case 0: return v.x;
        case 1: return v.y;
        case 2: return v.z;
        default: return 0; // 如果索引无效，返回0或其他错误码
    }
}

__forceinline__ __device__ float dot(float* a, float3 b) {
    return a[0] * b.x + a[1] * b.y + a[2] * b.z;
}

__forceinline__ __device__ float dot(float3 a, float* b) {
...
__forceinline__ __device__ float3 min(const float3& a, const float limit) {
    return make_float3(min(a.x, limit), min(a.y, limit), min(a.z, limit));
}

__forceinline__ __device__ float intersect(
	const float3& x,
	const float3& n,
	const float3& d)
{
	const float t = dot(n, x) / dot(n, d);
	float p_z = t * d.z;
